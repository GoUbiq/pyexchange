...
class ExchangeServiceSOAP(object):
    ...
    def send(self, xml, mailbox_address=None, headers=None, retries=4, timeout=30, encoding="utf-8"):
        ...
        log.info(etree.tostring(
            request_xml, encoding=encoding, pretty_print=True))
        response = self._send_soap_request(
            request_xml, headers=headers, retries=retries, timeout=timeout, encoding=encoding)
    ...
    def _parse(self, response, encoding="utf-8"):
        ...
        try:
            ...
            raise FailedExchangeException(
                u"Unable to parse response from Exchange - check your login information. Error: %s" % err)
    ...
    def _check_for_SOAP_fault(self, xml_tree):
        # Check for SOAP errors. if <soap:Fault> is anywhere in the response,
        # flip out
    ...
    def _wrap_soap_xml_request(self, exchange_xml, mailbox_address=None):
        if mailbox_address is None:
            root = S.Envelope(
                S.Header(self._exchange_header()), S.Body(exchange_xml))
        else:
            root = S.Envelope(S.Header(self._exchange_header(), T.ExchangeImpersonation(T.ConnectingSID(T.SmtpAddress(
                mailbox_address))), T.TimeZoneContext(T.TimeZoneDefinition({u'Id': u'UTC'}))), S.Body(exchange_xml))
    ...
    def _parse_date_only_naive(self, date_string):
        date = datetime.strptime(
            date_string[0:10], self.EXCHANGE_DATE_FORMAT[0:8])
    ...
    def _xpath_to_dict(self, element, property_map, namespace_map):
        ...
        for key in property_map:
            ...
            log.info(u'Pulling xpath {xpath} into key {key}'.format(
                key=key, xpath=item[u'xpath']))
            ...
            if nodes:
                ...
                for node in nodes:
                    ...
                    if cast_as == u'datetime':
                        ...
                        result_for_node.append(
                            self._parse_date_only_naive(node.text))
                    elif cast_as == u'int':
