...
ExchangeEventOrganizer = namedtuple(
    'ExchangeEventOrganizer', ['name', 'email'])
ExchangeEventAttendee = namedtuple('ExchangeEventAttendee', [
                                   'name', 'email', 'required'])
...
RESPONSES = [RESPONSE_ACCEPTED, RESPONSE_DECLINED,
             RESPONSE_TENTATIVE, RESPONSE_UNKNOWN]
...
class BaseExchangeCalendarEvent(object):
    ...
    # any attributes that have changed, and we need to update in Exchange
    ...
    WEEKLY_DAYS = [u'Sunday', u'Monday', u'Tuesday',
    ...
    def attendees(self):
        ...
        # this is redundant in python 2, but necessary in python 3 - .values()
        # returns dict_values, not a list
    ...
    def add_attendees(self, attendees, required=True):
        ...
        *attendees* can be a list of email addresses or :class:`ExchangeEventAttendee` objects.
        ...
        new_attendees = self._build_resource_dictionary(
            attendees, required=required)
    ...
    def validate(self):
        ...
        if self.reminder_minutes_before_start and not isinstance(self.reminder_minutes_before_start, int):
            raise TypeError(
                "reminder_minutes_before_start must be of type int")
    ...
    def _build_resource_dictionary(self, resources, required=True):
        result = {}
        if resources:
            item_list = resources if isinstance(
                resources, list) else [resources]
            for item in item_list:
                if isinstance(item, ExchangeEventAttendee):
                    if item.email is None:
                        raise ValueError(
                            u"You tried to add a resource or attendee with a blank email: {0}".format(item))

                    result[item.email] = ExchangeEventResponse(
                        email=item.email, required=item.required, name=item.name, response=None, last_response=None)
                    ...
                    if item is None:
                        raise ValueError(
                            u"You tried to add a resource or attendee with a blank email.")

                    result[item] = ExchangeEventResponse(
                        email=item, required=required, name=None, response=None, last_response=None)
