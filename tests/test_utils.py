...
def test_converting_non_tz_aware_date_returns_tz_aware():
    ...
    assert convert_datetime_to_utc(utc_time) == datetime(
        year=2014, month=1, day=1, hour=1, minute=1, second=1, tzinfo=utc)
...
def test_converting_tz_aware_date_returns_tz_aware_date():
    ...
    # argument of the standard datetime constructors does not work for
    # timezones with DST.
    pacific_time = timezone(
        "US/Pacific").localize(datetime(year=2014, month=4, day=1, hour=1, minute=0, second=0))
    utc_time = utc.localize(
        datetime(year=2014, month=4, day=1, hour=8, minute=0, second=0))
