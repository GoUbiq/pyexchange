...
class Test_ParseEventResponseData(unittest.TestCase):
    ...
    def test_conflicting_events(self):
        ...
        assert conflicting_events[
            0].calendar_id == TEST_CONFLICT_EVENT.calendar_id
...
class Test_GetOccurence(unittest.TestCase):
    ...
    def test_get_daily_event_occurrences(self):
        ...
        for occ in range(len(occurrences)):
            ...
            assert occurrences[
                occ].subject == TEST_EVENT_DAILY_OCCURRENCES[occ].subject
            assert occurrences[
                occ].location == TEST_EVENT_DAILY_OCCURRENCES[occ].location
            assert occurrences[
                occ].start == TEST_EVENT_DAILY_OCCURRENCES[occ].start
            assert occurrences[
                occ].end == TEST_EVENT_DAILY_OCCURRENCES[occ].end
            assert occurrences[
                occ].body == TEST_EVENT_DAILY_OCCURRENCES[occ].body
            assert occurrences[occ].calendar_id == TEST_EVENT_DAILY_OCCURRENCES[
                occ].calendar_id
