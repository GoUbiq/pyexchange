...
class Test_PopulatingANewEvent(unittest.TestCase):
    ...
    def test_can_add_attendees_via_named_tuple(self):
        ...
        person = ExchangeEventAttendee(name=PERSON_OPTIONAL_ACCEPTED.name,
                                       email=PERSON_OPTIONAL_ACCEPTED.email, required=PERSON_OPTIONAL_ACCEPTED.required)
    ...
    def test_can_assign_to_required_attendees(self):
        ...
        event.required_attendees = [
            PERSON_REQUIRED_ACCEPTED.email, PERSON_OPTIONAL_ACCEPTED.email]
    ...
    def test_can_assign_to_optional_attendees(self):
        ...
        assert event.required_attendees[
            0].email == PERSON_REQUIRED_ACCEPTED.email
        assert event.optional_attendees[
            0].email == PERSON_OPTIONAL_ACCEPTED.email
...
class Test_CreatingANewEvent(unittest.TestCase):
    ...
    def setUp(self):
        self.event = self.service.calendar().event(
            start=TEST_EVENT.start, end=TEST_EVENT.end)
    ...
    def test_attendees_must_have_an_email_address_take1(self):
        ...
        with raises(ValueError):
            self.event.add_attendees(ExchangeEventAttendee(
                name="Bomb", email=None, required=True))
            self.event.create()
    ...
    def test_can_set_subject(self):
        ...
        assert TEST_EVENT.subject in HTTPretty.last_request.body.decode(
            'utf-8')
    ...
    def test_can_set_location(self):
        ...
        assert TEST_EVENT.location in HTTPretty.last_request.body.decode(
            'utf-8')
    ...
    def test_start_time(self):
        ...
        assert TEST_EVENT.start.strftime(
            EXCHANGE_DATE_FORMAT) in HTTPretty.last_request.body.decode('utf-8')
    ...
    def test_end_time(self):
        ...
        assert TEST_EVENT.end.strftime(
            EXCHANGE_DATE_FORMAT) in HTTPretty.last_request.body.decode('utf-8')
    ...
    def test_attendees(self):
        ...
        attendees = [PERSON_REQUIRED_ACCEPTED.email,
                     PERSON_REQUIRED_TENTATIVE.email]
